...
// Path to the contract class artifacts
...
const COMPILED_CONTRACT_CLASS_PATH = path.join(__dirname, '../contracts/target/dev/region_verifier_UltraKeccakZKHonkVerifier.compiled_contract_class.json');

async function main() {
  try {
    // 1. Load contract class artifacts
    const contractClass = JSON.parse(fs.readFileSync(CONTRACT_CLASS_PATH, 'utf-8'));
    const compiledContractClass = JSON.parse(fs.readFileSync(COMPILED_CONTRACT_CLASS_PATH, 'utf-8'));
